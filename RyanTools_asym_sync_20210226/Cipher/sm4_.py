"""
SM4 GM
@author: Dawei
"""
import copy

# Expanded SM4 S-boxes    Sbox table: 8bits input convert to 8 bits output
SboxTable = [
    
]

# System parameter
FK = [0xa3b1bac6, 0x56aa3350, 0x677d9197, 0xb27022dc]

# fixed parameter
CK = [
    0x00070e15, 0x1c232a31, 0x383f464d, 0x545b6269,
    0x70777e85, 0x8c939aa1, 0xa8afb6bd, 0xc4cbd2d9,
    0xe0e7eef5, 0xfc030a11, 0x181f262d, 0x343b4249,
    0x50575e65, 0x6c737a81, 0x888f969d, 0xa4abb2b9,
    0xc0c7ced5, 0xdce3eaf1, 0xf8ff060d, 0x141b2229,
    0x30373e45, 0x4c535a61, 0x686f767d, 0x848b9299,
    0xa0a7aeb5, 0xbcc3cad1, 0xd8dfe6ed, 0xf4fb0209,
    0x10171e25, 0x2c333a41, 0x484f565d, 0x646b7279
]

ENCRYPT = 0
DECRYPT = 1


def GET_UINT32_BE(key_data):
    tmp_data = int((key_data[0] << 24) | (key_data[1] << 16) | (key_data[2] << 8) | (key_data[3]))
    return tmp_data


def PUT_UINT32_BE(n):
    return [int((n >> 24) & 0xff), int((n >> 16) & 0xff), int((n >> 8) & 0xff), int(n & 0xff)]


# rotate shift left marco definition
def SHL(x, n):
    xx = int(int(x << n) & 0xffffffff)
    return xx


def ROTL(x, n):
    xx = SHL(x, n)
    yy = xx | int((x >> (32 - n)) & 0xffffffff)
    return yy


def XOR(a, b):
    return list(map(lambda x, y: x ^ y, a, b))


def sm4Sbox(idx):
    """
    look up in SboxTable and get the related value.
    :param idx: [in] inch: 0x00~0xFF (8 bits unsigned value).
    :return: Sbox value.
    """
    return SboxTable[idx]


def sm4CalciRK(ka):
    """
    Calculating round encryption key.
    :param ka: [in] a: a is a 32 bits unsigned value;
    :return: sk[i]: i{0,1,2,3,...31}.
    """
    b = [0, 0, 0, 0]
    a = PUT_UINT32_BE(ka)
    b[0] = sm4Sbox(a[0])
    b[1] = sm4Sbox(a[1])
    b[2] = sm4Sbox(a[2])
    b[3] = sm4Sbox(a[3])
    bb = GET_UINT32_BE(b[0:4])
    rk = bb ^ (ROTL(bb, 13)) ^ (ROTL(bb, 23))
    return rk


def sm4Lt(ka):
    """
    private F(Lt) function:
    "T algorithm" == "L algorithm" + "t algorithm".
    :param ka: [in] a: a is a 32 bits unsigned value;
    :return: c: c is calculated with line algorithm "L" and nonline algorithm "t"
    """
    b = [0, 0, 0, 0]
    a = PUT_UINT32_BE(ka)
    b[0] = sm4Sbox(a[0])
    b[1] = sm4Sbox(a[1])
    b[2] = sm4Sbox(a[2])
    b[3] = sm4Sbox(a[3])
    bb = GET_UINT32_BE(b[0:4])
    c = bb ^ (ROTL(bb, 2)) ^ (ROTL(bb, 10)) ^ (ROTL(bb, 18)) ^ (ROTL(bb, 24))
    return c


def sm4F(x0, x1, x2, x3, rk):
    """
    private F function:
    Calculating and getting encryption/decryption contents.
    :param x0: [in] x0: original contents;
    :param x1: [in] x1: original contents;
    :param x2: [in] x2: original contents;
    :param x3: [in] x3: original contents;
    :param rk: [in] rk: encryption/decryption key;
    :return: the contents of encryption/decryption contents.
    """
    return x0 ^ sm4Lt(x1 ^ x2 ^ x3 ^ rk)


class Sm4(object):
    def __init__(self):
        self.sk = [0] * 32
        self.mode = ENCRYPT

    def sm4_set_key(self, key_data, mode):
        self.sm4_setkey(key_data, mode)

    def sm4_setkey(self, key, mode):
        MK = [0, 0, 0, 0]
        k = [0] * 36
        MK[0] = GET_UINT32_BE(key[0:4])
        MK[1] = GET_UINT32_BE(key[4:8])
        MK[2] = GET_UINT32_BE(key[8:12])
        MK[3] = GET_UINT32_BE(key[12:16])
        k[0:4] = XOR(MK[0:4], FK[0:4])
        for i in range(32):
            k[i + 4] = k[i] ^ (sm4CalciRK(k[i + 1] ^ k[i + 2] ^ k[i + 3] ^ CK[i]))
            self.sk[i] = k[i + 4]
        # print(k)
        # print(self.sk)
        self.mode = mode
        if mode == DECRYPT:
            for idx in range(16):
                t = self.sk[idx]
                self.sk[idx] = self.sk[31 - idx]
                self.sk[31 - idx] = t

    def sm4_one_round(self, sk, in_put):
        out_put = []
        ulbuf = [0] * 36
        ulbuf[0] = GET_UINT32_BE(in_put[0:4])
        ulbuf[1] = GET_UINT32_BE(in_put[4:8])
        ulbuf[2] = GET_UINT32_BE(in_put[8:12])
        ulbuf[3] = GET_UINT32_BE(in_put[12:16])
        for idx in range(32):
            ulbuf[idx + 4] = sm4F(ulbuf[idx], ulbuf[idx + 1], ulbuf[idx + 2], ulbuf[idx + 3], sk[idx])

        out_put += PUT_UINT32_BE(ulbuf[35])
        out_put += PUT_UINT32_BE(ulbuf[34])
        out_put += PUT_UINT32_BE(ulbuf[33])
        out_put += PUT_UINT32_BE(ulbuf[32])
        # print(out_put)
        return out_put

    def sm4_crypt_ecb(self, input_data):
        """SM4-ECB block encryption/decryption"""
        length = len(input_data)
        i = 0
        lst_output_data = []
        while length > 0:
            lst_output_data += self.sm4_one_round(self.sk, input_data[i:i + 16])
            i += 16
            length -= 16

        lst_output_data_hex = ['{:02x}'.format(a) for a in lst_output_data]
        str_output_hex = ''.join(lst_output_data_hex)
        return lst_output_data, str_output_hex

    def sm4_crypt_cbc(self, iv, input_data):
        """SM4-CBC buffer encryption/decryption"""
        length = len(input_data)
        i = 0
        lst_output_data = []
        tmp_input = [0] * 16
        if self.mode == ENCRYPT:
            while length > 0:
                tmp_input[0:16] = XOR(input_data[i:i + 16], iv[0:16])
                lst_output_data += self.sm4_one_round(self.sk, tmp_input[0:16])
                iv = copy.deepcopy(lst_output_data[i:i + 16])
                i += 16
                length -= 16
        else:
            while length > 0:
                lst_output_data += self.sm4_one_round(self.sk, input_data[i:i + 16])
                lst_output_data[i:i + 16] = XOR(lst_output_data[i:i + 16], iv[0:16])
                iv = copy.deepcopy(input_data[i:i + 16])
                i += 16
                length -= 16

        lst_output_data_hex = ['{:02x}'.format(a) for a in lst_output_data]
        str_output_hex = ''.join(lst_output_data_hex)
        return lst_output_data, str_output_hex


def sm4_crypt_ecb(mode, key, data):
    sm4_d = Sm4()
    sm4_d.sm4_set_key(key, mode)
    en_data = sm4_d.sm4_crypt_ecb(data)
    return en_data


def sm4_crypt_cbc(mode, key, iv, data):
    sm4_d = Sm4()
    sm4_d.sm4_set_key(key, mode)
    en_data = sm4_d.sm4_crypt_cbc(iv, data)
    return en_data


if __name__ == "__main__":
    key_data = [0x11] * 16
    input_data = [0x31] * 16
    iv_data = [0] * 16

    sm4_d = Sm4()
    print(key_data)

    sm4_d.sm4_set_key(key_data, ENCRYPT)
    en_data, en_dat_hexstr = sm4_d.sm4_crypt_ecb(input_data)
    print("en_data:", en_data)
    print("en_dat_hexstr:", en_dat_hexstr)

    sm4_d.sm4_set_key(key_data, DECRYPT)
    de_data, de_dat_hexstr = sm4_d.sm4_crypt_ecb(en_data)
    print("de_data:", de_data)
    print("de_dat_hexstr:", de_dat_hexstr)

    if de_data == input_data:
        print("ecb check pass")
    else:
        print("ecb check fail")

    sm4_d.sm4_set_key(key_data, ENCRYPT)
    en_data, en_dat_hexstr = sm4_d.sm4_crypt_cbc(iv_data, input_data)
    print("en_data:", en_data)
    print("en_dat_hexstr:", en_dat_hexstr)

    sm4_d.sm4_set_key(key_data, DECRYPT)
    de_data, de_data_hexstr = sm4_d.sm4_crypt_cbc(iv_data, en_data)
    print("de_data:", de_data)
    print("de_data_hexstr:", de_data_hexstr)
    if de_data == input_data:
        print("cbc check pass")
    else:
        print("cbc check fail")
